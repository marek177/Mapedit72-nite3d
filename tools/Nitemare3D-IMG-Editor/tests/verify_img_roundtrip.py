...
def load(path):
    ...
    while pos < len(data):
        ...
        # Convert column-major file -> row-major.
        pix = bytearray(w*h)
        k = 0
        for x in range(w):
            for y in range(h):
...
def main(paths):
    for name in paths:
        ...
        # Synthetic size change: add one column to the first image if possible.
        h2,e2=load(name)
        old_second=e2[1][0]
        old=e2[0]
        ...
        if w<255:
            ...
            # The changed file must still parse to the same number of records.
