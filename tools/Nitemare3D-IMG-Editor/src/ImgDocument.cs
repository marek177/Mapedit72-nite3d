...
public sealed class ImgDocument
{
    ...
    public static ImgDocument Load(string path)
    {
        ...
        if (firstOffset < 12 || firstOffset >= data.Length)
            throw new InvalidDataException(
                $"Invalid first image offset {firstOffset}.");
        ...
        while (pos < data.Length)
        {
            if (pos + 10 > data.Length)
                throw new InvalidDataException(
                    $"Truncated image header at 0x{pos:X}.");

            byte w = data[pos];
            byte h = data[pos + 1];
            if (w == 0 || h == 0)
                throw new InvalidDataException(
                    $"Invalid zero-sized image #{index} at 0x{pos:X}.");

            int pixelCount = checked(w * h);
            int recordSize = checked(10 + pixelCount);
            if (pos + recordSize > data.Length)
                throw new InvalidDataException(
                    $"Image #{index} extends past end of file.");
            ...
            // File layout is column-major: for x, then for y.
            int src = pos + 10;
            for (int x = 0; x < w; x++)
            {
                for (int y = 0; y < h; y++)
    ...
    public void SaveAs(string path, bool makeBackup = true)
    {
        ...
        // Compute new physical offsets first.
        ...
        for (int i = 0; i < Entries.Count; i++)
        {
            ...
            if (e.OriginalOffset > 0 &&
                e.OriginalOffset <= uint.MaxValue &&
                cursor <= uint.MaxValue)
            {
        ...
        if (cursor > int.MaxValue)
            throw new InvalidDataException(
                "IMG file would exceed this editor's 2 GB safety limit.");

        // Preserve every unknown header byte. Only exact DWORD values that
        // reference a known old image start are relocated.
        byte[] newHeader = (byte[])Header.Clone();
        for (int i = 0; i + 4 <= newHeader.Length; i += 4)
        {
            uint oldValue = BinaryPrimitives.ReadUInt32LittleEndian(
                newHeader.AsSpan(i, 4));

            if (relocation.TryGetValue(oldValue, out uint newValue))
            {
                BinaryPrimitives.WriteUInt32LittleEndian(
                    newHeader.AsSpan(i, 4), newValue);
            }
        }
            ...
            temp, FileMode.Create, FileAccess.Write, FileShare.None))
        {
            ...
            foreach (ImgEntry e in Entries)
            {
                ...
                // Convert row-major editor pixels back to IMG column-major.
